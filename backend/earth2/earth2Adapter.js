...
const OUTPUT = path.resolve('backend/earth2/lastForecast.json');
export function getEarth2Status() {
  ...
  return {
    ...
    summary: latestRun?.summary ?? (configured ? 'Earth-2 bridge configured but no cached forecast found.' : 'Earth-2 slot available. Set LUCY_EARTH2_ENABLED=1 and connect a local Earth2Studio bridge.'),
