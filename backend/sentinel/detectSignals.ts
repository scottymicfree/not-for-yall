...
function getLevel(value: number): 'normal' | 'watch' | 'warning' {
...
function buildGovernanceTrend(ledgerEntries: Array<{timestamp:number}>, emmaReviews: Array<{decision:'approved'|'rejected'}>) {
  const now = Date.now(); const recentWindowMs = 5 * 60 * 1000;
...
export function detectSignals(
  ...
  emmaReviews: Array<any> = [],
) {
  const simA = twinEarth.simA.earth; const simB = twinEarth.simB.earth;
